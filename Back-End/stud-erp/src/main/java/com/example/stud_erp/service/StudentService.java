...
public class StudentService {
    ...
    public List<Student> getStudentsByDepartment(String departmentName) {
        // Since major field is used for department, filter by major
        System.out.println("🎯 StudentService: Looking for major = '" + departmentName + "'");

        // First, let's see what majors actually exist in the database
        List<Student> allStudents = studentRepository.findAll();
        System.out.println("📈 Total students in database: " + allStudents.size());

        if (allStudents.size() > 0) {
            System.out.println("🏫 Available majors in database:");
            allStudents.stream()
                    .map(Student::getMajor)
                    .distinct()
                    .forEach(major -> System.out.println("   - '" + major + "'"));
        }

        List<Student> result = studentRepository.findByMajor(departmentName);
        System.out.println("✅ Students found for '" + departmentName + "': " + result.size());
        return result;
    }
}
