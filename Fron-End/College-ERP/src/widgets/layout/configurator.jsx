...
export function Configurator() {
  ...
  const { openConfigurator, sidenavColor, sidenavType, fixedNavbar, theme } =
    controller;
  const [stars, setStars] = React.useState(0);

  const toggleTheme = () => {
    setTheme(dispatch, theme === "dark" ? "light" : "dark");
  };
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    const fetchStars = async () => {
      try {
        const response = await fetch("https://api.github.com/repos/creativetimofficial/material-tailwind-dashboard-react");
        if (response.ok) {
          const data = await response.json();
          setStars(formatNumber(data.stargazers_count, 1));
        } else {
          console.log("Unable to fetch GitHub stars (offline or API unavailable)");
        }
      } catch (error) {
        console.log("GitHub API unavailable (offline mode)");
        // Set a default value when offline
        setStars("1.2K");
      }
    };
